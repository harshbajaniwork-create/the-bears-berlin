...
const AfroHealthProject = () => {
  const project = projects.find((p) => p.id === "afrohealth");
  const [imagesLoaded, setImagesLoaded] = useState(false);
  const [contentLoaded, setContentLoaded] = useState(false);

  const { isLoading } = usePageLoader([imagesLoaded, contentLoaded], 1400);
          ...
          <div className="mb-16 md:mb-24 max-w-7xl">
            <div className="space-y-6 md:space-y-8">
              ...
              <div className="flex justify-between items-center border-b border-gray-300 dark:border-gray-600 pb-4 md:pb-6">
                ...
                {isMobile ? (
                  <p className="text-sm sm:text-base md:text-lg lg:text-xl text-right font-medium text-gray-500 dark:text-gray-400">
                    Brand Strategy, Storytelling,
                    <br className="block sm:hidden" /> Creative Direction,{" "}
                    <br className="hidden sm:block" /> Campaign Production
                  </p>
                ) : (
                  <p className="text-sm sm:text-base md:text-lg lg:text-xl font-medium text-gray-500 dark:text-gray-400">
                    {project?.scopeOfWork}
...
export default AfroHealthProject;
