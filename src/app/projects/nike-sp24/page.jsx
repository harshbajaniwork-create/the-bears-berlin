...
const NikeSp24Project = () => {
  const project = projects.find((p) => p.id === "nike-sp24");
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-8 lg:px-12 py-8 md:py-16 pt-20 sm:pt-32 md:pt-40">
          ...
          <div className="flex justify-center mb-16 md:mb-24">
            <div className="h-12 sm:h-16 md:h-20 flex items-center">
              <img
                ...
                className="h-full scale-200 object-contain filter-none dark:invert transition-all duration-300"
                loading="eager"
                decoding="async"
                onError={(e) => {
          ...
          {project?.productImages && project.productImages.length > 0 && (
            <div className="mb-6 md:mb-32 max-w-7xl mx-auto">
              <div className="grid grid-cols-1 sm:grid-cols-2  gap-4 md:gap-6 lg:gap-8">
                {project.productImages.slice(0, 2).map((imageSrc, index) => (
                  <div
                    key={index}
          ...
          {project?.productImages && project.productImages.length > 3 && (
            <div className="mb-6 md:mb-16 max-w-7xl mx-auto">
              <div className="grid grid-cols-1 sm:grid-cols-2  gap-4 md:gap-6 lg:gap-8">
                {project.productImages.slice(2).map((imageSrc, index) => (
                  <div
                    key={index + 3}
                    className=" overflow-hidden rounded-lg bg-gray-100 dark:bg-gray-800"
                  >
        ...
        {project?.videos && project.videos.length > 1 && (
          <div className="mb-20 md:mb-32 max-w-6xl mx-auto">
            <div className="space-y-8 md:space-y-12">
              {project.videos.slice(1, 3).map((videoId, index) => (
                <div
                  key={index}
                  className="relative aspect-video overflow-hidden rounded-lg md:rounded-xl bg-gray-100 dark:bg-gray-800"
                >
                  <iframe
                    src={`https://customer-64sz73htfhb823gb.cloudflarestream.com/${videoId}/iframe?poster=https%3A%2F%2Fcustomer-64sz73htfhb823gb.cloudflarestream.com%2F${videoId}%2Fthumbnails%2Fthumbnail.jpg%3Ftime%3D%26height%3D600&autoplay=true&loop=true&muted=true`}
                    loading="lazy"
                    style={{
                      border: "none",
                      position: "absolute",
...
export default NikeSp24Project;
